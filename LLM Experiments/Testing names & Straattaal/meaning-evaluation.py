...
# Download Dutch extension of WordNet if not yet done
try:
...
def check_response(file_name: str, output_file: str):
    ...
    def check_presence(row, meaning_column):
        ...
        # temporarily remove the Straattaal words so it doesn't match itself (f.e. voetoe and voet)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python meaning-evaluation.py <response file name> <output name>")
        sys.exit(1)
